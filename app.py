...
os.makedirs(DB_DIR, exist_ok=True) # This ensures the Database folder actually exists
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(DB_DIR, "portal.log"), encoding="utf-8"),
...
SQLITE_PATH = os.path.join(DB_DIR, "portal.db")
...
def  _build_cache():
    ...
    try:
        ...
        hpc_list = q(f"""
            ...
            lm_true_farmers AS (
                SELECT hpc_plant_key, COUNT(DISTINCT farmer_code) AS lm_total_farmers
                FROM proc_monthly_farmer
                WHERE (yr * 100 + mth) = (
            ...
            SELECT s.hpc_plant_key, s.hpc_name, s.plant_name, s.plant_code, s.region, s.zone,
                   ...
                   COALESCE(tf.distinct_farmers, 0) AS mtd_farmers_total,
                   COALESCE(lmf.lm_total_farmers, 0) AS lm_farmers,
                   ROUND(s.mtd_avg_fat,3) AS mtd_avg_fat,
                   ROUND(s.mtd_rate,2)    AS mtd_rate,
                   ROUND(s.mtd_payout,0)  AS mtd_payout,
                   s.has_mtd_data,
                   ROUND(CASE WHEN s.has_mtd_data = 1
                              THEN s.mtd
                              ELSE s.lm
                         END, 1)                                    AS effective_mtd,
                   ROUND(CASE WHEN s.has_mtd_data = 1
                              THEN COALESCE(tf.distinct_farmers, 0)
                              ELSE COALESCE(lmf.lm_total_farmers, 0)
                         END, 0)                                    AS effective_farmers,
                   ROUND(CASE WHEN s.has_mtd_data = 1 AND s.mtd_avg_fat > 0
                              THEN s.mtd_avg_fat
                              ELSE s.lm_avg_fat
                         END, 3)                                    AS effective_fat,
                   ROUND(CASE WHEN s.has_mtd_data = 1
                              THEN NULL
                              ELSE s.lm_avg_snf
                         END, 3)                                    AS lm_avg_snf_fallback,
                   ROUND(CASE WHEN s.has_mtd_data = 1
                              THEN NULL
                              ELSE s.lm_cost_per_fat_kg
                         END, 2)                                    AS lm_cost_fallback,
                   s.has_mtd_data,
                   ROUND(s.lm,1)          AS lm_lpd
            FROM proc_period_snapshot s
            LEFT JOIN true_farmers tf ON s.hpc_plant_key = tf.hpc_plant_key
            LEFT JOIN lm_true_farmers lmf ON s.hpc_plant_key = lmf.hpc_plant_key
...
""")
        ...
        snap_row = conn.execute(f"""
            SELECT
                -- HPCs that have uploaded this cycle → real LPD
                -- HPCs that haven't uploaded yet     → use last month's LPD
                -- This stops the company total crashing in the first half of
                -- every month when 15-day-cycle regions have no data yet.
                SUM(CASE WHEN has_mtd_data = 1 THEN mtd ELSE lm END)    AS mtd,
                SUM(lm)            AS lm,
                SUM(lmtd)          AS lmtd,
                SUM(lymtd)         AS lymtd,

                -- Separate numerator/denominator for YoY — only HPCs where
                -- BOTH this year AND last year have data. This stops Region B
                -- (not uploaded yet this cycle) from dragging YoY to -40%.
                SUM(CASE WHEN has_mtd_data=1 AND lymtd>0 THEN mtd   END) AS mtd_for_yoy,
                SUM(CASE WHEN has_mtd_data=1 AND lymtd>0 THEN lymtd END) AS lymtd_for_yoy,
                ...
                -- Dark = had data last month, zero THIS month AND has uploaded (not just waiting)
                SUM(CASE WHEN mtd=0 AND lm>0 AND has_mtd_data=1
                    THEN 1 ELSE 0 END)                                    AS dark_hpcs,
                ...
                SUM(has_mtd_data)                                         AS hpcs_with_data,
                COUNT(*)                                                  AS total_snapshot_hpcs
            FROM proc_period_snapshot
            WHERE snapshot_date='{snap_date}'
        """).fetchone()
        ...
        mtd_for_yoy   = snap_row["mtd_for_yoy"]   or 0
        lymtd_for_yoy = snap_row["lymtd_for_yoy"] or 0
        ...
        snapshot = {
            ...
            "lmtd":           round(lmtd,  0),
            "lymtd":          round(lymtd, 0),
            "yoy_pct": round((mtd_for_yoy - lymtd_for_yoy) / lymtd_for_yoy * 100, 1) if lymtd_for_yoy else 0,
            "data_coverage_pct": round((snap_row["hpcs_with_data"] or 0) / (snap_row["total_snapshot_hpcs"] or 1) * 100, 0),
...
def _build_cache_bg():
    ...
    with _cache_lock:
        if _cache_building:
            log.info("Cache already building — skipping")
            return
...
def _recompute_snapshot(hpcs, mhpc, base_snap):
    ...
    Uses effective_mtd (imputed with lm for regions not yet uploaded)
    so ZH/RH totals don't crash to zero mid-cycle.
    """
    if not hpcs:
        return base_snap

    def effective_mtd(h):
        # If region has uploaded this cycle, use real mtd
        # If not uploaded yet, use last month as placeholder
        if h.get("has_mtd_data", 1):
            return h.get("mtd", 0) or 0
        else:
            return h.get("lm", 0) or 0

    mtd   = sum(effective_mtd(h)        for h in hpcs)
    ...
    # YoY only counts HPCs where BOTH this year AND last year have data
    mtd_for_yoy   = sum(
        effective_mtd(h) for h in hpcs
        if h.get("has_mtd_data", 1) and (h.get("lymtd") or 0) > 0
    )
    lymtd_for_yoy = sum(
        h.get("lymtd", 0) or 0 for h in hpcs
        if h.get("has_mtd_data", 1) and (h.get("lymtd") or 0) > 0
    )
    ...
    return {
        **base_snap,
        ...
        "lmtd":           round(lmtd,  0),
        "lymtd":          round(lymtd, 0),
        "yoy_pct":        round((mtd_for_yoy - lymtd_for_yoy) / lymtd_for_yoy * 100, 1)
                          if lymtd_for_yoy else 0,
        ...
        # Dark = region HAS uploaded but this specific HPC sent nothing
        # A 15-day HPC that hasn't uploaded yet is NOT dark
        "dark_hpcs": len({
            h.get("hpc_plant_key") for h in hpcs
            if h.get("has_mtd_data", 1) and not h.get("mtd") and h.get("lm")
        }),
        "total_hpcs": len({
            h.get("hpc_plant_key") for h in hpcs
            if h.get("mtd") or h.get("lm")
        }),
...
def do_login():
    ...
    # READ — close immediately, don't hold connection open
    conn = get_db()
    user = conn.execute(
    ...
    # WRITE — non-critical, never block a successful login
    try:
        wconn = get_db()
        wconn.execute("UPDATE users SET last_login=datetime('now') WHERE id=?", (user["id"],))
        wconn.commit()
        wconn.close()
    except sqlite3.OperationalError as e:
        log.warning(f"last_login update skipped for {user['username']}: {e}")
...
# ── PRODUCTION INITIALIZATION (Runs for WSGI/Hypercorn workers) ──────────────
...
def initialize_worker():
    try:
        ...
        log.warning(f"Worker DB init skipped (likely locked by another worker): {e}")
        
    # Kick off the background cache build immediately when the worker boots
    _build_cache_bg()

# Run initialization as soon as the module loads
initialize_worker()
...
# python -m hypercorn app:app --bind 0.0.0.0:8091 --certfile "D:\certs\7db886ac63f4bfb8.pem" --keyfile "D:\certs\privateKey.key" --workers 4