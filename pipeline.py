...
# ── CONFIG ────────────────────────────────────────────────────────────────────
# Dynamically get the folder where pipeline.py is located
BASE_DIR = Path(__file__).parent.absolute()

PARQUET_DIR  = BASE_DIR / "Database" / "parquet"
SQLITE_PATH  = str(BASE_DIR / "Database" / "portal.db")
TMP_DIR      = BASE_DIR / "Database" / "tmp"
...
# ── BATCH SIZE CONFIG ─────────────────────────────────────────────────────────
# How many days to pull in one SQL Server query.
# 7 days × ~110K rows/day = ~770K rows per query — safe on 8GB RAM.
# Increase to 14 if your server has 16GB+ RAM.
# Never set above 30 — SQL Server IN() clause gets slow above that.
FETCH_BATCH_DAYS = 7


def _fetch_batch(dates, sql_conn, progress_cb=None, batch_num=1, total_batches=1):
    """
    Fetch a list of dates (max FETCH_BATCH_DAYS) in ONE SQL Server query.
    Splits the result into one parquet file per day.
    Returns list of (date, row_count) for dates that had data.

    Why batched and not one-per-day:
      One-per-day = N round trips to SQL Server. On a server with 50ms latency,
      15 days = 15 × 50ms overhead + query time. Very slow.

    Why batched and not all-at-once:
      All-at-once on 200M rows would crash RAM. 7-day batches keep memory
      under ~200MB per batch regardless of data volume.
    """
    if not dates:
        return []

    # Skip dates we already have parquets for
    missing = [d for d in dates if not (PARQUET_DIR / f"proc_{d}.parquet").exists()]
    if not missing:
        log.info(f"  Batch {batch_num}/{total_batches}: all {len(dates)} parquets exist, skipping")
        return []

    date_list = ", ".join(f"'{d}'" for d in missing)
    label = f"{missing[0]} to {missing[-1]}" if len(missing) > 1 else str(missing[0])

    if progress_cb:
        progress_cb("fetch", batch_num, total_batches,
                    f"Fetching batch {batch_num}/{total_batches}: {label}")

    log.info(f"  Batch {batch_num}/{total_batches}: querying {len(missing)} days ({label})")
    ...
    SELECT
        ...
        CAST([Net Price]       AS FLOAT)                                      AS net_price
    FROM [HeritageIT].[P&I].[fact_procurement_transactions]
    WHERE CAST([MCS Date] AS DATE) IN ({date_list})
    """

    try:
        df = pd.read_sql(sql, sql_conn)
    except Exception as e:
        log.error(f"  Batch query failed: {e}")
        raise

    if df.empty:
        log.warning(f"  Batch {batch_num}: query returned 0 rows")
        return []

    log.info(f"  Batch {batch_num}: {len(df):,} rows received")

    # Split into one parquet per day
    df['proc_date'] = pd.to_datetime(df['proc_date']).dt.date
    results = []

    for d in missing:
        day_df = df[df['proc_date'] == d]
        if day_df.empty:
            log.warning(f"    {d}: no rows in batch result — skipping parquet")
            continue
        path = PARQUET_DIR / f"proc_{d}.parquet"
        day_df.to_parquet(path, index=False, compression="snappy")
        log.info(f"    {d}: {len(day_df):,} rows → {path.name}")
        results.append((d, len(day_df)))

    # Explicitly free memory — important on 8GB RAM servers
    del df
    return results
...
def build_snapshot(conn=None, progress_cb=None):
    ...
    Pay-cycle fix baked in:
      - MTD/LMTD/LYMTD denominators use per-REGION active upload days,
        not the global calendar day_num.
      - has_mtd_data=1 for HPCs whose region HAS uploaded this cycle.
      - has_mtd_data=0 for HPCs whose region has NOT uploaded yet.
      - The app.py aggregate layer uses has_mtd_data to impute LM for
        not-yet-uploaded regions, preventing the company KPI from crashing.
    """
    own = conn is None
    if own: conn = get_db()
    ...
    -- Using REGION (not HPC) means:
    --   • A 10-day-cycle region gets ÷10, a 15-day-cycle region gets ÷15  (Problem 1 fix)
    --   • An offline HPC still gets ÷region_days, so its low numerator
    --     correctly shows up as low LPD — the outage is not hidden        (Problem 2 fix)
    --   • A region with 0 uploads this cycle gets active_days=NULL,
    --     which becomes has_mtd_data=0 in the mtd CTE — the app layer
    --     will impute LM for these HPCs in the company total             (Problem 3 fix)
    region_days_mtd AS (
        SELECT h.region,
               COUNT(DISTINCT DATE(d.proc_date)) AS active_days
        FROM proc_daily_hpc d
        JOIN hpc_dim h ON d.hpc_plant_key = h.hpc_plant_key
        WHERE DATE(d.proc_date) BETWEEN '{curr_start}' AND '{snap_date}'
        GROUP BY h.region
    ),
    region_days_lmtd AS (
        SELECT h.region,
        ...
        FROM proc_daily_hpc d
        JOIN hpc_dim h ON d.hpc_plant_key = h.hpc_plant_key
        WHERE DATE(d.proc_date) BETWEEN '{prev_start}' AND '{lmtd_end}'
        GROUP BY h.region
    ),
    region_days_lymtd AS (
        SELECT h.region,
               COUNT(DISTINCT DATE(d.proc_date)) AS active_days
        FROM proc_daily_hpc d
        JOIN hpc_dim h ON d.hpc_plant_key = h.hpc_plant_key
        WHERE DATE(d.proc_date) BETWEEN '{date(ly_yr, curr_mth, 1)}' AND '{ly_mtd_end}'
        GROUP BY h.region
    ),

    -- MTD: divides by region's actual upload days, not global day_num
    -- has_mtd_data = 1 means this HPC's region HAS uploaded this cycle
    -- has_mtd_data = 0 (set via LEFT JOIN miss) means region hasn't uploaded yet
    mtd AS (
        SELECT
            d.hpc_plant_key,
            SUM(d.qty_ltr)      / NULLIF(r.active_days, 0)             AS mtd,
            SUM(d.farmer_count) / NULLIF(r.active_days, 0)             AS mtd_farmers,
            ROUND(SUM(d.total_fat_kg)/NULLIF(SUM(d.qty_kg),0)*100, 3)  AS mtd_avg_fat,
            SUM(d.total_net_price)                                      AS mtd_payout,
            SUM(d.total_net_price) / NULLIF(SUM(d.qty_ltr), 0)         AS mtd_rate,
            COALESCE(r.active_days, 0)                                  AS region_active_days,
            1                                                           AS has_mtd_data
        FROM proc_daily_hpc d
        JOIN hpc_dim h         ON d.hpc_plant_key = h.hpc_plant_key
        JOIN region_days_mtd r ON h.region        = r.region
        WHERE DATE(d.proc_date) BETWEEN '{curr_start}' AND '{snap_date}'
        GROUP BY d.hpc_plant_key
    ),

    -- LM: last month completed period — also compute fat/snf/cost
    -- These are used as fallback values for 15-day cycle HPCs that
    -- haven't uploaded current month data yet
    lm AS (
        SELECT hpc_plant_key,
            ...
            ROUND(SUM(total_fat_kg)/NULLIF(SUM(qty_kg),0)*100, 3)       AS lm_avg_fat,
            ROUND(SUM(total_snf_kg)/NULLIF(SUM(qty_kg),0)*100, 3)       AS lm_avg_snf,
            ROUND(SUM(total_net_price)/NULLIF(SUM(total_fat_kg),0), 2)  AS lm_cost_per_fat_kg
        FROM proc_daily_hpc
        WHERE DATE(proc_date) BETWEEN '{prev_start}' AND '{prev_end}'
        GROUP BY hpc_plant_key
    ),

    -- LMTD: last month same-day window, uses region upload days
    lmtd AS (
        SELECT
            d.hpc_plant_key,
            SUM(d.qty_ltr)      / NULLIF(r.active_days, 0)     AS lmtd,
            SUM(d.farmer_count) / NULLIF(r.active_days, 0)     AS lmtd_farmers
        FROM proc_daily_hpc d
        JOIN hpc_dim h          ON d.hpc_plant_key = h.hpc_plant_key
        JOIN region_days_lmtd r ON h.region        = r.region
        WHERE DATE(d.proc_date) BETWEEN '{prev_start}' AND '{lmtd_end}'
        GROUP BY d.hpc_plant_key
    ),

    -- LYMTD: last year same period, also uses region upload days
    -- (last year's region may have had a different cycle length too)
    lymtd AS (
        SELECT
            d.hpc_plant_key,
            SUM(d.qty_ltr)      / NULLIF(r.active_days, 0)             AS lymtd,
            SUM(d.farmer_count) / NULLIF(r.active_days, 0)             AS lymtd_farmers,
            ROUND(SUM(d.total_fat_kg)/NULLIF(SUM(d.qty_kg),0)*100, 3)  AS lymtd_avg_fat
        FROM proc_daily_hpc d
        JOIN hpc_dim h           ON d.hpc_plant_key = h.hpc_plant_key
        JOIN region_days_lymtd r ON h.region        = r.region
        WHERE DATE(d.proc_date) BETWEEN '{date(ly_yr, curr_mth, 1)}' AND '{ly_mtd_end}'
        GROUP BY d.hpc_plant_key
    ),

    -- LYSM: last year full same month (unchanged)
    lysm AS (
        SELECT hpc_plant_key,
    ...
    SELECT
        ...
        lm.lm_avg_fat,
        lm.lm_avg_snf,
        lm.lm_cost_per_fat_kg,
        COALESCE(m.region_active_days, 0)  AS region_active_days,
        COALESCE(m.has_mtd_data,       0)  AS has_mtd_data,
...
def _ensure_tables(conn):
    ...
    # ── Column migrations — safe to run repeatedly, errors are ignored ────────
    for table, col in [
    ...
    # ── Pay-cycle fix columns — added here so any new server gets them
    #    automatically without needing a manual migration script ───────────────
    for col, default in [
        ('region_active_days',    '0'),
        ('has_mtd_data',          '0'),
        ('lm_avg_fat',            'NULL'),
        ('lm_avg_snf',            'NULL'),
        ('lm_cost_per_fat_kg',    'NULL'),
    ]:
        try:
            conn.execute(
                f"ALTER TABLE proc_period_snapshot "
                f"ADD COLUMN {col} INTEGER DEFAULT {default}"
            )
            conn.commit()
            log.info(f"Migrated proc_period_snapshot — added {col}")
        except Exception:
